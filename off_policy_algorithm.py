...
from a_class import BaseAlgorithm
...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def _dump_logs(self) -> None:
        ...
        #print(self.ep_info_buffer[-1])
        if len(self.ep_info_buffer) > 0 and len(self.ep_info_buffer[0]) > 0:
            for i in self.ep_info_buffer[0]:
                self.logger.record(f"rollout/ep_rew_mean_player{i}", safe_mean([ep_info[i]["r"] for ep_info in self.ep_info_buffer]))
                self.logger.record(f"rollout/ep_len_mean_player{i}", safe_mean([ep_info[i]["l"] for ep_info in self.ep_info_buffer]))
