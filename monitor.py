...
class Monitor(gym.Wrapper[ObsType, ActType, ObsType, ActType]):
    ...
    def __init__(
        ...
        self.rewards: List[Any] = [] #Marco: float -> Any
        self.needs_reset = True
        self.episode_returns: List[Any] = []  #Marco: float -> Any
    ...
    def step(self, action: ActType) -> Tuple[ObsType, SupportsFloat, bool, bool, Dict[str, Any]]:
        ...
        self.rewards.append(reward) #mod
        if terminated or truncated:
            self.needs_reset = True
            ep_info = dict()
            # ep_rew = sum(self.rewards)
            for reward_key in reward:
                ep_rew = sum([r[reward_key] for r in self.rewards]) #mod
                ep_len = len(self.rewards)
                ep_rew /= ep_len
                ep_info[reward_key] = {"r": round(ep_rew, 6), "l": ep_len, "t": round(time.time() - self.t_start, 6)}
